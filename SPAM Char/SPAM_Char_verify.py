...
def analyze_output(num_qubits, num_exps, qubit_set, parallel_run, runnum, foldername):
    ...
    np.save(foldername + "/alpha_arr_"+str(runnum), alpha_arr)
    np.save(foldername + "/alpha_m_arr_"+str(runnum), alpha_m_arr)
    np.save(foldername + "/alpha_sp_arr_"+str(runnum), alpha_sp_arr)
    np.save(foldername + "/delta_arr_"+str(runnum), delta_arr)
    np.save(foldername + "/qubit_set_"+str(runnum), np.array(qubit_set))
...
foldername = backend_name+'_disjoint_verify_'+str(num_qubits)+'q_SPAMjobs_1'
...
for i in phi_list:
    exp_list_phi = []
    for j in qubit_set:
        exp_list_phi.append(SPAMParams(physical_qubits=(j,), backend=backend, num_exps=num_exps, phi=i, analysis=SPAMParamsAnalysis()))
    exp_list.append(exp_list_phi)

parallel_exp_list = []
for i in range(len(exp_list)):
    parallel_exp_list.append(ParallelExperiment(exp_list[i]))
# run the experiment
results = []
for i in range(len(parallel_exp_list)):
