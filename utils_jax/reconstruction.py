...
def recon_S_12_12(coefs, **kwargs):
    ...
    tb = np.linspace(0, T, 10**4)
    y_arr = np.array([make_y(tb, ['CPMG', 'CDD1-1/2'], ctime=c_times[i], M=1) for i in range(np.size(c_times))])
    for i in range(np.size(c_times)):
        for j in range(np.size(c_times)):
            U[i, j] = ((M/T)*np.abs(ff(y_arr[i][0, 0], tb, wk[j]))**2).round(18)
    return np.linalg.inv(U)@(C_1_0_MT+C_2_0_MT-C_12_0_MT_4)*0.5
...
def recon_S_1_12(coefs, **kwargs):
    ...
    y1_arr = np.array([make_y(tb, ['CDD1', 'FID'], ctime=c_times[i], M=1) for i in range(np.size(c_times))])
    y2_arr = np.array([make_y(tb, ['CDD1', 'CDD1-1/2'], ctime=c_times[i], M=1) for i in range(np.size(c_times))])
    for i in range(np.size(c_times)):
        for j in range(np.size(c_times)):
            U1[i, j]=((2*M/T)*ff(y1_arr[i][2, 2], tb, -wk[j])*ff(y1_arr[i][0, 0], tb, wk[j]))
            U2[i, j]=((2*M/T)*ff(y2_arr[i][2, 2], tb, -wk[j])*ff(y2_arr[i][0, 0], tb, wk[j]))
...
def recon_S_2_12(coefs, **kwargs):
    ...
    U1 = np.zeros((np.size(c_times), np.size(c_times)), dtype=np.complex128)
    U2 = np.zeros((np.size(c_times), np.size(c_times)), dtype=np.complex128)
    y1_arr = np.array([make_y(tb, ['FID', 'CDD1'], ctime=c_times[i], M=1) for i in range(np.size(c_times))])
    y2_arr = np.array([make_y(tb, ['CDD1-1/2', 'CDD1'], ctime=c_times[i], M=1) for i in range(np.size(c_times))])
    for i in range(np.size(c_times)):
        for j in range(np.size(c_times)):
            U1[i, j]=((2*M/T)*ff(y1_arr[i][2, 2], tb, -wk[j])*ff(y1_arr[i][1, 1], tb, wk[j]))
            U2[i, j]=((2*M/T)*ff(y2_arr[i][2, 2], tb, -wk[j])*ff(y2_arr[i][1, 1], tb, wk[j]))
