...
ax1.set_ylabel(r'$S^+_{1,1}(\omega_k)$(kHz)')
# remove x ticks
ax1.set_xticks([])
ax2.plot(wk/1e6, S_22_k/1e3, 'r.')
ax2.plot(w/1e6, S_11(w)/1e3, 'r--')
ax2.set_ylabel(r'$S^+_{2,2}(\omega_k)$(kHz)')
ax2.set_xticks([])
ax3.plot(wk/1e6, recon_S_12_12([C_1_0_MT_1, C_2_0_MT_1, C_12_0_MT_2], c_times=c_times, M=M, T=T)/1e3, 'r.')
ax3.set_ylabel(r'$S^+_{12,12}(4\omega_k)$(kHz)')
ax3.set_ylim(0,1)
ax3.set_xticks([])
...
ax4.set_ylabel(r'$S^+_{1,2}(\omega_k)$(kHz)')
